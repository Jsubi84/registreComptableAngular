...
export class CategoryComponent {
  ...
  constructor(
    ...
    private subCat_service: SubcategoriaService,
  ...
  Delete(categoria: Categoria) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.subCat_service
          .ckSubCatToDeleteCategoria(categoria.id)
          .subscribe((data) => {
            if (data == 0) {
              ...
              this.dialog.simpleAlert(
                'La categoria no es pot borrar ja que te ' +
                  data +
                  ' subcategorias associades',
                'Categoria no borrable',
                'info'
              );
            }
          });
